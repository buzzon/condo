...
import { Mumu, MumuWhereInput } from '../../../schema'

export const getMumuCreateMessage = (intl,ticket) => {
    if (!ticket) {
        return
    }
...
export const getMumuTitleMessage = (intl, ticket) => {
    if (!ticket) {
        return
    }
...
export const getMumuFormattedLastStatusUpdate = (intl, ticket) => {
    if (!ticket) {
        return
    }
...
export interface IFilters extends Pick<Mumu, 'name' | 'isWorked'> {
    status?: Array<string>
    assignee?: string
    executor?: string
    property?: string
...
export const filtersToQuery = (filters: IFilters): MumuWhereInput => {
    ...
    const isWorked = get(filters, 'isWorked')
